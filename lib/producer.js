...
var createChannel = function() {
  ...
  .then(function (_amqpChannel) {
    ...
    while(reqQueue.length > 0) {
      reqQueue.shift().resolve(amqpChannel);
    }
...
var produce = function(_queue, _msg, _options) {
  ...
  if (!connected) {
    return connect().then(function(channel) {
      return channel.sendToQueue(_queue, _msg, _options)
    });
  } else {
    return Promise.resolve(amqpChannel.sendToQueue(_queue, new Buffer(_msg), _options));
  }
};
