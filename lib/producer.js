...
function catchError(err) {
  if (err) Logger.error('[BMQ-PRODUCER] ', err.stack, '\n    occured while connecting');
  amqpIntervalId = clearInterval(amqpIntervalId);
  amqpChannel = undefined;
  amqpConnection = undefined;
  connecting = false;
...
function createRpcQueue(queue) {
  if (rpcQueues[queue].queue && !reconnection) return Promise.resolve();
  ...
  .then(function (_queue) {
    var prefetch = config.prefetch || process.env.AMQP_PREFETCH || 1;
    prefetch = (typeof prefetch !== 'number') ? parseInt(prefetch) : prefetch;
    amqpChannel.prefetch(prefetch);
...
var produce = function(_queue, _msg, _options) {
  if (!connected) {
    utils.pushIfNotExist('producer', productions, {
      queue: _queue,
      options: _options,
      msg: _msg
    });
